import React from "react";
import "../styles/FacebookInstaAd.css";
const FacebookInstaAd = () => {
  return (
    <div class="wink snipcss-plfI7">
      <a href="#front-door-content" class="skipToContent-3Sx-7">
        Skip to content
      </a>
      <div class="frontDoorRow-2yluE">
        <div class="navigation-2_SUj navigationOpenDesktop-1gZ8o">
          <div class="navigationBack-3AH9o navigationBackOpenDesktop-2jERI">
            <button
              class="navigationBackButton-1JmON"
              type="button"
              aria-label="Exit"
            >
              <div class="navigationBackButtonArrow-1c_Nj">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon navigationBackButtonArrowIcon-36OSq"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M13.293 6.293l1.414 1.414L10.414 12l4.293 4.293-1.414 1.414L7.586 12l5.707-5.707z"
                  ></path>
                </svg>
              </div>
              <img
                class="navigationBackButtonFreddie-qXKoK"
                src="data:image/svg+xml;base64,"
                alt=""
              />
              <div class="navigationBackButtonCross-3bL01">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon navigationBackButtonCrossIcon-i_WP4"
                >
                  <path d="M12 13.414l6.293 6.293 1.414-1.414L13.414 12l6.293-6.293-1.414-1.414L12 10.586 5.707 4.293 4.293 5.707 10.586 12l-6.293 6.293 1.414 1.414L12 13.414z"></path>
                </svg>
              </div>
            </button>
          </div>
          <div
            role="list"
            class="navigationButtons-3OyNw navigationButtonsOpenDesktop-1t_7K"
          >
            <div role="listitem" class="container-IL_ti">
              <button
                aria-expanded="true"
                type="button"
                class="button-1YrMi"
                aria-controls="mc:41"
              >
                <p class="root-3TDqk medium-3AcAC text-2t4Eg">Email</p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                  ></path>
                </svg>
              </button>
              <div>
                <div id="style-viphG" class="style-viphG">
                  <ul id="mc:41">
                    <div role="listitem">
                      <div class="container-IL_ti">
                        <a
                          aria-current="page"
                          class="label-g01vr isSelected-LmGnC isChild-Zr4Q5"
                          id="option-d3pmf"
                          aria-describedby="mc:74"
                          href="#/create-campaign/explore/emailCampaign"
                        >
                          <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                            Regular
                          </span>
                        </a>
                      </div>
                      <a href="#front-door-content" class="skipToContent-3Sx-7">
                        Skip to Regular content
                      </a>
                    </div>
                    <div role="listitem">
                      <div class="container-IL_ti">
                        <a
                          class="label-g01vr isChild-Zr4Q5"
                          id="option-c15rz"
                          aria-describedby="mc:75"
                          href="#/create-campaign/explore/emailCampaign:plainText"
                        >
                          <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                            Plain text
                          </span>
                        </a>
                      </div>
                      <a href="#front-door-content" class="skipToContent-3Sx-7">
                        Skip to Plain text content
                      </a>
                    </div>
                    <div role="listitem">
                      <div class="container-IL_ti">
                        <a
                          class="label-g01vr isChild-Zr4Q5"
                          id="option-rcxm0"
                          aria-describedby="mc:76"
                          href="#/create-campaign/explore/emailCampaign:multivariate"
                        >
                          <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                            Multivariate
                          </span>
                        </a>
                      </div>
                      <a href="#front-door-content" class="skipToContent-3Sx-7">
                        Skip to Multivariate content
                      </a>
                    </div>
                    <div role="listitem">
                      <div class="container-IL_ti">
                        <a
                          class="label-g01vr isChild-Zr4Q5"
                          id="option-nkl59"
                          aria-describedby="mc:77"
                          href="#/create-campaign/explore/emailCampaign:template"
                        >
                          <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                            Template
                          </span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24"
                            focusable="false"
                            aria-hidden="true"
                            class="wink-icon"
                          >
                            <path
                              fill-rule="evenodd"
                              clip-rule="evenodd"
                              d="M20.707 7.707L10 18.414l-5.207-5.207 1.414-1.414L10 15.586l9.293-9.293 1.414 1.414z"
                            ></path>
                          </svg>
                        </a>
                      </div>
                      <a href="#front-door-content" class="skipToContent-3Sx-7">
                        Skip to Template content
                      </a>
                    </div>
                  </ul>
                </div>
              </div>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-ky9b8"
                  aria-describedby="mc:42"
                  href="#/create-campaign/explore/automation"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg">
                    Automations
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-c3hs3"
                  aria-describedby="mc:43"
                  href="#/create-campaign/explore/website"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg">
                    Website
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-6yeyq"
                  aria-describedby="mc:44"
                  href="#/create-campaign/explore/page"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg">
                    Landing Page
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-04b49"
                  aria-describedby="mc:45"
                  href="#/create-campaign/explore/creative-assistant"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg">
                    Creative Assistant
                  </span>
                </a>
              </div>
            </div>
            <hr role="presentation" class="navigationDivider-kfIYM" />
            <div role="listitem" class="container-IL_ti">
              <button
                aria-expanded="false"
                type="button"
                class="button-1YrMi"
                aria-controls="mc:46"
              >
                <p class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                  Signup Form
                </p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                  ></path>
                </svg>
              </button>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-5zzla"
                  aria-describedby="mc:47"
                  href="#/create-campaign/explore/survey"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                    Surveys
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem" class="container-IL_ti">
              <button
                aria-expanded="false"
                type="button"
                class="button-1YrMi"
                aria-controls="mc:48"
              >
                <p class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                  Ad
                </p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                  ></path>
                </svg>
              </button>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-8ytay"
                  aria-describedby="mc:49"
                  href="#/create-campaign/explore/social"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                    Social Post
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem" class="container-IL_ti">
              <button
                aria-expanded="false"
                type="button"
                class="button-1YrMi"
                aria-controls="mc:50"
              >
                <p class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                  Postcard
                </p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                  ></path>
                </svg>
              </button>
            </div>
          </div>
        </div>
        <div class="content-1h3tW snipcss0-0-0-1 snipcss-U3rGd">
          <div
            class="stack-1qp4V contentText-2ARRa contentTextOpenDesktop-3FWWK snipcss-VT3QR"
            id="front-door-content"
          >
            <div class="container-1xp3H">
              <h1 class="heading-1-2zF42 heading-vLHyh root-PihPG">
                Create a Facebook / Instagram ad
              </h1>
              <p class="root-3TDqk medium-3AcAC subHeading-1b7r4">
                Reach specific contacts, find new ones, or target your website
                visitors with Facebook and Instagram ads.
              </p>
              <form
                class="stack-1qp4V spacing6-nznRY"
                novalidate=""
                id="form-buthq"
              >
                <div class="root-2RFjU emptyAudience-13cLy">
                  <div class="before-2EDUs">
                    <label class="mcds-label-default" id="mc:207" for="mc:206">
                      Internal ad name
                    </label>
                  </div>
                  <div class="inputWrapper-TVkf7">
                    <input
                      type="text"
                      id="mc:206"
                      aria-invalid="false"
                      name="textValue"
                      value=""
                    />
                  </div>
                </div>
                <fieldset class="fieldset-3ISha">
                  <legend class="mcds-label-default">
                    Special Ad Category
                  </legend>
                  <div class="root-1olwd selectCheckbox-3ukoA">
                    <input
                      class="input-2v40B"
                      id="mc:213"
                      type="checkbox"
                      indeterminate="false"
                      aria-describedby="mc:215"
                      aria-checked="false"
                      name="checkboxValue"
                    />
                    <div class="text-vFMr3">
                      <label id="mc:214" for="mc:213" class="">
                        I'm creating a campaign for ads in a special ad
                        category;
                        <br />
                        Ads related to credit, employment or housing
                      </label>
                      <div id="mc:215" class="description-3q6yK">
                        <div class="root-1cS4q selectCheckboxValue-3YqvL">
                          <div class="before-fjpii">
                            <label
                              class="mcds-label-default"
                              id="mc:218"
                              for="mc:217"
                            >
                              Category
                            </label>
                          </div>
                          <div class="selectWrapper-1gG6j">
                            <select
                              disabled=""
                              id="mc:217"
                              name="selectCheckboxValue"
                            >
                              <option value="0">Credit</option>
                              <option value="1">Employment</option>
                              <option value="2">Housing</option>
                            </select>
                            <div class="indicator-3GTSs">
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 24 24"
                                focusable="false"
                                aria-hidden="true"
                                class="wink-icon"
                              >
                                <path
                                  fill-rule="evenodd"
                                  clip-rule="evenodd"
                                  d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                                ></path>
                              </svg>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </fieldset>
                <div class="button-1iD64">
                  <button
                    class="root-sBgFt container-3-bH7 primary-33czz button-3mfLr"
                    type="submit"
                  >
                    <span class="temporarySpan-2iF2p">Begin</span>
                  </button>
                </div>
              </form>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default FacebookInstaAd;
