import React from "react";
import "../styles/GoogleMarktingAd.css";
const GoogleMarktingAd = () => {
  return (
    <div class="wink snipcss-plfI7">
      <a href="#front-door-content" class="skipToContent-3Sx-7">
        Skip to content
      </a>
      <div class="frontDoorRow-2yluE">
        <div class="navigation-2_SUj navigationOpenDesktop-1gZ8o">
          <div class="navigationBack-3AH9o navigationBackOpenDesktop-2jERI">
            <button
              class="navigationBackButton-1JmON"
              type="button"
              aria-label="Exit"
            >
              <div class="navigationBackButtonArrow-1c_Nj">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon navigationBackButtonArrowIcon-36OSq"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M13.293 6.293l1.414 1.414L10.414 12l4.293 4.293-1.414 1.414L7.586 12l5.707-5.707z"
                  ></path>
                </svg>
              </div>
              <img
                class="navigationBackButtonFreddie-qXKoK"
                src="data:image/svg+xml;base64,"
                alt=""
              />
              <div class="navigationBackButtonCross-3bL01">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon navigationBackButtonCrossIcon-i_WP4"
                >
                  <path d="M12 13.414l6.293 6.293 1.414-1.414L13.414 12l6.293-6.293-1.414-1.414L12 10.586 5.707 4.293 4.293 5.707 10.586 12l-6.293 6.293 1.414 1.414L12 13.414z"></path>
                </svg>
              </div>
            </button>
          </div>
          <div
            role="list"
            class="navigationButtons-3OyNw navigationButtonsOpenDesktop-1t_7K"
          >
            <div role="listitem" class="container-IL_ti">
              <button
                aria-expanded="true"
                type="button"
                class="button-1YrMi"
                aria-controls="mc:41"
              >
                <p class="root-3TDqk medium-3AcAC text-2t4Eg">Email</p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                  ></path>
                </svg>
              </button>
              <div>
                <div id="style-viphG" class="style-viphG">
                  <ul id="mc:41">
                    <div role="listitem">
                      <div class="container-IL_ti">
                        <a
                          aria-current="page"
                          class="label-g01vr isSelected-LmGnC isChild-Zr4Q5"
                          id="option-d3pmf"
                          aria-describedby="mc:74"
                          href="#/create-campaign/explore/emailCampaign"
                        >
                          <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                            Regular
                          </span>
                        </a>
                      </div>
                      <a href="#front-door-content" class="skipToContent-3Sx-7">
                        Skip to Regular content
                      </a>
                    </div>
                    <div role="listitem">
                      <div class="container-IL_ti">
                        <a
                          class="label-g01vr isChild-Zr4Q5"
                          id="option-c15rz"
                          aria-describedby="mc:75"
                          href="#/create-campaign/explore/emailCampaign:plainText"
                        >
                          <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                            Plain text
                          </span>
                        </a>
                      </div>
                      <a href="#front-door-content" class="skipToContent-3Sx-7">
                        Skip to Plain text content
                      </a>
                    </div>
                    <div role="listitem">
                      <div class="container-IL_ti">
                        <a
                          class="label-g01vr isChild-Zr4Q5"
                          id="option-rcxm0"
                          aria-describedby="mc:76"
                          href="#/create-campaign/explore/emailCampaign:multivariate"
                        >
                          <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                            Multivariate
                          </span>
                        </a>
                      </div>
                      <a href="#front-door-content" class="skipToContent-3Sx-7">
                        Skip to Multivariate content
                      </a>
                    </div>
                    <div role="listitem">
                      <div class="container-IL_ti">
                        <a
                          class="label-g01vr isChild-Zr4Q5"
                          id="option-nkl59"
                          aria-describedby="mc:77"
                          href="#/create-campaign/explore/emailCampaign:template"
                        >
                          <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                            Template
                          </span>
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24"
                            focusable="false"
                            aria-hidden="true"
                            class="wink-icon"
                          >
                            <path
                              fill-rule="evenodd"
                              clip-rule="evenodd"
                              d="M20.707 7.707L10 18.414l-5.207-5.207 1.414-1.414L10 15.586l9.293-9.293 1.414 1.414z"
                            ></path>
                          </svg>
                        </a>
                      </div>
                      <a href="#front-door-content" class="skipToContent-3Sx-7">
                        Skip to Template content
                      </a>
                    </div>
                  </ul>
                </div>
              </div>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-ky9b8"
                  aria-describedby="mc:42"
                  href="#/create-campaign/explore/automation"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg">
                    Automations
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-c3hs3"
                  aria-describedby="mc:43"
                  href="#/create-campaign/explore/website"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg">
                    Website
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-6yeyq"
                  aria-describedby="mc:44"
                  href="#/create-campaign/explore/page"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg">
                    Landing Page
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-04b49"
                  aria-describedby="mc:45"
                  href="#/create-campaign/explore/creative-assistant"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg">
                    Creative Assistant
                  </span>
                </a>
              </div>
            </div>
            <hr role="presentation" class="navigationDivider-kfIYM" />
            <div role="listitem" class="container-IL_ti">
              <button
                aria-expanded="false"
                type="button"
                class="button-1YrMi"
                aria-controls="mc:46"
              >
                <p class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                  Signup Form
                </p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                  ></path>
                </svg>
              </button>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-5zzla"
                  aria-describedby="mc:47"
                  href="#/create-campaign/explore/survey"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                    Surveys
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem" class="container-IL_ti">
              <button
                aria-expanded="false"
                type="button"
                class="button-1YrMi"
                aria-controls="mc:48"
              >
                <p class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                  Ad
                </p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                  ></path>
                </svg>
              </button>
            </div>
            <div role="listitem">
              <div class="container-IL_ti">
                <a
                  class="label-g01vr"
                  id="option-8ytay"
                  aria-describedby="mc:49"
                  href="#/create-campaign/explore/social"
                >
                  <span class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                    Social Post
                  </span>
                </a>
              </div>
            </div>
            <div role="listitem" class="container-IL_ti">
              <button
                aria-expanded="false"
                type="button"
                class="button-1YrMi"
                aria-controls="mc:50"
              >
                <p class="root-3TDqk medium-3AcAC text-2t4Eg secondaryText-71Q7m">
                  Postcard
                </p>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  focusable="false"
                  aria-hidden="true"
                  class="wink-icon"
                >
                  <path
                    fill-rule="evenodd"
                    clip-rule="evenodd"
                    d="M6.293 10.707l1.414-1.414L12 13.586l4.293-4.293 1.414 1.414L12 16.414l-5.707-5.707z"
                  ></path>
                </svg>
              </button>
            </div>
          </div>
        </div>
        <div class="content-1h3tW snipcss0-0-0-1 snipcss-U3rGd">
          <div
            class="stack-1qp4V contentText-2ARRa contentTextOpenDesktop-3FWWK snipcss0-0-0-1 snipcss-M7aff"
            id="front-door-content"
          >
            <div class="container-1xp3H">
              <h1 class="heading-1-2zF42 heading-vLHyh root-PihPG">
                Create a Google remarketing ad
              </h1>
              <p class="root-3TDqk medium-3AcAC subHeading-1b7r4">
                Recapture your audience's attention with the help of Google’s
                huge ad network. Encourage your potential customers to come back
                to your site with web retargeting ads that display your brand or
                products on the sites they browse everyday.
              </p>
              <form
                class="stack-1qp4V spacing6-nznRY"
                novalidate=""
                id="form-xtx5d"
              >
                <div class="root-W0b4G warning-3wFdA selectNoOptionsMessage-2Vx-W cluster-3D5Qr nowrap-34OZ-">
                  <div class="alignItemsCenter-1HCiJ justifyFlexStart-ejJl1 spacing4-1S_zR">
                    <span class="icon-3fYw1">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        focusable="false"
                        aria-hidden="true"
                        class="wink-icon"
                      >
                        <path d="M13.5 17a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zm-3-12l.5 9h2l.5-9h-3z"></path>
                        <path
                          fill-rule="evenodd"
                          clip-rule="evenodd"
                          d="M12 23c6.075 0 11-4.925 11-11S18.075 1 12 1 1 5.925 1 12s4.925 11 11 11zm0-2a9 9 0 100-18 9 9 0 000 18z"
                        ></path>
                      </svg>
                    </span>
                    <div class="content-2tUie cluster-3D5Qr nowrap-34OZ-">
                      <div class="alignItemsCenter-1HCiJ justifySpaceBetween-2M_OY spacing2-3-fWQ">
                        <div class="copy-1hjCu">
                          <p class="root-3TDqk small-bold-6R-6E">
                            Connect your site
                          </p>
                          <span class="root-3TDqk small-secondary-3_Rq2">
                            Remarketing ads help you attract the attention of
                            customers who have previously visited your site or
                            e-commerce store.{" "}
                            <a href="/account/connected-sites/app-selection?referrer_url=%2F%23%2Fcreate-campaign%2Fexplore%2FadCampaign%3AremarketingAd&amp;feature_title=Google%20remarketing%20ad">
                              Connect your site
                            </a>{" "}
                            to create an ad, or learn how{" "}
                            <a
                              href="https://mailchimp.com/help/about-connected-sites/?_ga=2.253774795.250632254.1618850880-1333050263.1535578299"
                              target="_blank"
                              rel="noopener noreferrer"
                            >
                              site connection
                            </a>{" "}
                            works.
                          </span>
                        </div>
                        <div class="cluster-3D5Qr nowrap-34OZ-">
                          <div class="alignItemsCenter-1HCiJ justifyFlexEnd-3_ERd spacing4-1S_zR"></div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </form>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default GoogleMarktingAd;
